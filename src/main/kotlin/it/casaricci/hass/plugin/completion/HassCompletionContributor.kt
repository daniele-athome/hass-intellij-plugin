...
internal class HassCompletionContributor : CompletionContributor() {
    init {
        extend(
            ...
            YamlElementPatternHelper.getSingleLineScalarKey("icon"),
            MdiIconCompletionProvider(),
        )
        extend(
            CompletionType.BASIC,
            YamlElementPatternHelper.getScalarValueWithTagPrefix(HASS_TOKEN_SECRET),
            SecretCompletionProvider(),
        )
        extend(
            CompletionType.BASIC,
            YamlElementPatternHelper.getSingleLineScalarKey("action", "service"),
            HassActionCompletionProvider(),
        )
        extend(
            CompletionType.BASIC,
            PlatformPatterns.or(
    ...
    /**
     * In theory this is useful only for token-based completions (for now only icons), because other
     * contributors use [PsiElement] objects: in those cases, the IDE will know how to replace the
     * whole identifier.
     */
    override fun beforeCompletion(context: CompletionInitializationContext) {
        ...
        if (scalarElement != null) {
            ...
            context.offsetMap.addOffset(
                CompletionInitializationContext.IDENTIFIER_END_OFFSET,
                endPos,
            )
        }
    }
