...
class HassActionCompletionProvider : CompletionProvider<CompletionParameters>() {
    public override fun addCompletions(
        ...
        // we should maybe cache LookupElement objects, but we don't have a way to purge them when
        // things change
            ...
            let {
                    when (action) {
                        is YAMLKeyValue -> {
                            ...
                            domainName?.let { entityId(domainName, action.keyText) }
                        }

                        is JsonProperty -> {
                            val domainName = getDomainNameFromActionName(action)
                            domainName?.let { entityId(domainName, action.name) }
                        }
